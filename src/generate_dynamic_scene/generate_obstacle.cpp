...
int main(int argc, char **argv) {
	...
	while (get_distance(current_point, goal_point) > 0.5 * static_cast<double>(loop_rate.expectedCycleTime().toSec()) * velocity) {
		...
		double time = count;
		current_point(0) = (goal_point(0) - start_point(0)) * (velocity * time) / distance + start_point(0);
		current_point(1) = (goal_point(1) - start_point(1)) * (velocity * time) / distance + start_point(1);

		std::cout << "The human current position is, x: " << current_point(0) << " y: " << current_point(1)
				  << std::endl;
